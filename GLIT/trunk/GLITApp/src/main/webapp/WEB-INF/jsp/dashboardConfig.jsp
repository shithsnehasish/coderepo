...
/* function validateForm(rowCount){
	 var reportName = document.forms["setConfig"]["reportName"].value;
	var chartBox = document.forms["setConfig"]["chartBox"].value;
	var criteriaBox = document.forms["setConfig"]["criteriaBox"].value;
	if(reportName == null || reportName == "" || chartBox == null || chartBox == "" || criteriaBox == null || criteriaBox == ""){
		alert("false");
		return false;
	}else{
		alert("true");
	...
	for(var i=0;i<=rowCount;i++){
		var reportName = document.forms["setConfig"]["formBox"+i].value;
			...
			<div>
				<div class="breadcrumb flat" style="width: 100%;">
					<a href="client.spring">Client Details</a> <a
						...
						<a href="dashboard.spring">Dashboard</a>
						<a href="#" class="active">Dashboard Configuration</a>
				</div>
...
<c:if test="${availableConfigCount > 0}">
<!-- onsubmit="return checkPriority()" -->
<form id="setConfig" name="setConfig" method="post" action="saveConfig.spring">
<div style="width: 95%; padding: 16px 0px 0px 66px">
...
<table>
<tr>
	<td><label>Dashboard Name : </label></td> <td><input type="text" id="dashboardName" name="dashboardName" class="required" /></td>
</tr>
<tr>
<td>&nbsp;</td><td>&nbsp;</td>
</tr>
<tr>
	<td><label>Questionaire : </label></td> 
	<td>
	<select id="questionaireData" name="questionaireData" class="questionaireData" onfocus="getQuestionaireValue()" onchange="getForms()">
		<option value='0'>-- Select Questionaire --</option>
		<c:forEach items="${questionaires}" var="questionaire">
			<option value="${questionaire.questionaireId}">${questionaire.name}</option>
		</c:forEach>
	</select>
	</td>
</tr>
...
<tr>
<td><input id="reportName" name="reportName" type="text" class="required reportName" placeholder="Report Name" /></td>
<td><select id="formBox0" name="formBox" class="formBox" onchange="getField(0)">
<option value='0'>-- Select Form --</option>
</select></td>
<td><select id='fieldBox0' name ='fieldBox' class="fieldBox">
<option value='0'>-- Select Field --</option>
</select></td>
<td><select id="chartBox" name="chartBox" class="chartBox"><option value='0'>-- Select Chart --</option> <option value='Line Chart'>Line Chart</option> <option value='Bar Chart'>Bar Chart</option></select></td>
<td><select id="criteriaBox" name="criteriaBox" class="criteriaBox"><option value='0'>-- Select Criteria --</option><option value='Daily'>Daily</option> <option value='Weekly'>Weekly</option> <option value='Monthly'>Monthly</option> </select></td>
<!-- <td><select id='priorityBox0' name ='priorityBox'>
...
<td><a href="#" onclick="deleteRow(0)" style="text-decoration: none;">&nbsp;<img alt="" title="Remove Chart Configuartion " src="../GLITApp/images/chart_delete.png"/>  </a></td>
</tr>
</table>
</div>
<div class="panel-footer">
<a href="#" onclick="addRow(${availableConfigCount})" class="myButton"  style="text-decoration: none;">Add Chart</a>&nbsp;&nbsp;&nbsp;
<!-- <a href="#" onclick="deleteRow()">Remove Chart</a>&nbsp;&nbsp;&nbsp; -->
<a href="#" onclick="return validate(this);" class="myButton"  style="text-decoration: none;">Save Dashboard</a>&nbsp;&nbsp;&nbsp;
</div>
</div>
</div>
...
<div class="tab-content">
			<div class="tab-pane active" id="configList">
			<div style="width: 95%; padding: 16px 0px 0px 66px">
				<div class="panel panel-default" >
                        <div class="panel-heading">
				...
				<table class="table demo" data-page-size="2">
					...
					<tbody>
					<c:forEach items="${allDashboardConfig}" var="dashConf">
						<tr>
							<td>${dashConf.dashboardName}</td>
							<td>
							<table class="table demo" >
								...
								<c:forEach items="${dashConf.configUtils}" var="dash">
								<tr>
									<td>${dash.reportName}</td>
									<td>${dash.formName }</td>
									<td>${dash.fieldName }</td>
									<td>${dash.typeOfChart }</td>
									<td>${dash.criteria }</td>
									<td><a href="deleteDashboardConfigData.spring?dashboardConfigId=${dash.dashboardConfigId}" onclick="return confirm('Do you really want to delete Chart?')"><img alt="" title="Delete Chart" src="../GLITApp/images/chart-delete-icon.png"/></a></td>
								</tr>
								</c:forEach>
							</table>
							</td>
							<td>
							<a href="editDashboard.spring?dashboardId=${dashConf.dashboardId}"><img alt="" title="Edit Dashboard" src="../GLITApp/images/edit-dashboard.png"/></a>
							| <a href="deleteDashboard.spring?dashboardId=${dashConf.dashboardId}" onclick="return confirm('Do you really want to delete Dashboard with all the charts configuraion?')"><img alt="" title="Delete Dashboard" src="../GLITApp/images/delete-button.png"/></a></td>
						</tr>
						</c:forEach>
					</tbody>
				</table>
